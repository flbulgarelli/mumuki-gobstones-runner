...
(() => {
  ...
  const GobstonesTestRunner = new class {
    runTests(solution, exercise, result) {
      ...
      const runnerResults = runner.runTests({
        ...
        options: exercise.offline_test.options
      })

      result.status = runnerResults.status;
      result.mulangAst = runnerResults.mulangAst;
      result.testResults = runnerResults.results.map((it) => ({status: it.status, title: it.title, result: renderResult(it)}));
      // var options = {};
  ...
  $(document).ready(function () {
    mumuki.registerOfflineTestRunner(GobstonesTestRunner)
  });
